...
namespace ix
{
    ...
    bool Http::sendResponse(HttpResponsePtr response, std::unique_ptr<Socket>& socket)
    {
        ...
        // Write headers
        bool isBinary = response->binaryBuffer != nullptr;
        ss.str("");
        ss << "Content-Length: " << ((isBinary) ? response->binarySize : response->body.size()) << "\r\n";
        ...
        if(isBinary)
            return !response->binaryBuffer ? true : socket->writeBytes(response->binaryBuffer, response->binarySize, nullptr);
        else
